...
@section('title', 'مبيعات الجملة والموزعين | شورى إخوان')
@section('description', 'قسم التوزيع ومبيعات الجملة في شورى إخوان — أسعار تجارية مدروسة وحسومات خاصة للتجار والمقاولين والموزعين في كافة المحافظات السورية.')

@section('content')

    @php
        $govs = ['دمشق','ريف دمشق','حلب','حمص','حماة','اللاذقية','طرطوس','درعا','السويداء','دير الزور','الحسكة','الرقة','القنيطرة','إدلب'];
    ...
    <section class="py-20 lg:py-24 bg-white">
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
            <div class="text-center max-w-2xl mx-auto mb-14 reveal">
                <span class="inline-block text-brand font-bold text-sm mb-3">مزايا الشراكة</span>
                <h2 class="text-3xl sm:text-4xl font-black text-[#141414]">لماذا تختار الشراكة معنا؟</h2>
            </div>
    ...
    <section id="order" class="scroll-mt-24 py-20 lg:py-24 bg-[#f7f7f8] relative overflow-hidden">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-12 gap-10 lg:gap-14 items-start">
            ...
            <div class="lg:col-span-4 space-y-6 reveal">
                <div>
                    <span class="inline-block text-brand font-bold text-sm mb-3">تواصل مباشر</span>
                    <h2 class="text-2xl sm:text-3xl font-black text-[#141414] mb-4">تواصل مع قسم مبيعات الجملة</h2>
                    <p class="text-[#4b4b4b] text-sm leading-relaxed">
                        يرجى تعبئة النموذج وسيقوم مندوب المبيعات المختص بالتواصل معك فوراً. أو راسلنا مباشرة عبر واتساب.
                    </p>
                </div>

                @if ($whatsappDigits)
                    <a href="https://wa.me/{{ $whatsappDigits }}?text={{ urlencode('مرحباً شورى إخوان، أود الاستفسار عن مبيعات الجملة.') }}"
                ...
                <div class="bg-white rounded-2xl p-6 ring-1 ring-black/5 space-y-4">
                    <h3 class="font-bold text-[#141414] text-sm">صالاتنا ومستودعاتنا</h3>
            ...
            <div class="lg:col-span-8 reveal bg-white p-7 sm:p-10 rounded-[2rem] shadow-lg ring-1 ring-black/5">
                <h3 class="text-xl font-black text-[#141414] mb-2">نموذج طلب الشراء أو التسعير</h3>
                <p class="text-[#9a9a9a] text-sm mb-8">الحقول المعلّمة بـ <span class="text-brand">*</span> إلزامية.</p>
                ...
                <form id="wsForm" class="space-y-6 text-right">
                    ...
                    <div class="grid sm:grid-cols-2 gap-6">
                        <div>
                            <label for="ws_name" class="block text-xs font-bold text-[#141414] mb-2">الاسم / اسم الفعالية التجارية <span class="text-brand">*</span></label>
                            <input type="text" id="ws_name" required placeholder="مثال: مؤسسة الأمان الهندسية / السيد أحمد"
                        ...
                        <div>
                            <label for="ws_phone" class="block text-xs font-bold text-[#141414] mb-2">رقم الهاتف <span class="text-brand">*</span></label>
                    ...
                    <div class="grid sm:grid-cols-2 gap-6">
                        <div>
                            <label for="ws_product" class="block text-xs font-bold text-[#141414] mb-2">المنتجات المهتم بها <span class="text-brand">*</span></label>
                            <div class="relative">
                                <select id="ws_product" required
                                    ...
                                    @endforeach
                                    <option value="طلبية متنوعة / أخرى">طلبية متنوعة / أخرى</option>
                                </select>
                        ...
                        <div>
                            <label for="ws_gov" class="block text-xs font-bold text-[#141414] mb-2">المحافظة <span class="text-brand">*</span></label>
                            <div class="relative">
                    ...
                    {{-- العنوان --}}
                    <div>
                        <label for="ws_address" class="block text-xs font-bold text-[#141414] mb-2">عنوان المحل / المنطقة ضمن المحافظة <span class="text-brand">*</span></label>
                        <input type="text" id="ws_address" required placeholder="مثال: حلب - منطقة العرقوب الصناعية، أو دمشق - شارع الثورة"
                               class="w-full bg-[#f7f7f8] rounded-xl px-4 py-3 text-sm text-[#141414] focus:outline-none focus:ring-2 focus:ring-brand focus:bg-white transition-all">
                    </div>

                    {{-- التفاصيل --}}
                    <div>
                        <label for="ws_message" class="block text-xs font-bold text-[#141414] mb-2">تفاصيل الطلبية أو الرسالة</label>
                        <textarea id="ws_message" rows="5" placeholder="اكتب هنا الكميات المطلوبة، أو أرقام الموديلات، أو أي استفسار حول الأسعار والوكالات"
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        .then(({ status, body }) => {
            ...
            okBox.innerHTML = 'شكراً لك! تم استلام طلبك بنجاح وتوجيهه إلى قسم مبيعات الجملة. سيتواصل معك مندوب المبيعات المختص في أقرب وقت.';
            okBox.classList.remove('hidden');
            okBox.scrollIntoView({ behavior: 'smooth', block: 'center' });

            // تمرير الطلب إلى تطبيق البريد معبّأً ببيانات النموذج
            const body =
                'الاسم / الفعالية التجارية: ' + data.name + '\n' +
                'رقم الهاتف: ' + data.phone + '\n' +
                'المنتجات المهتم بها: ' + data.product_interest + '\n' +
                'المحافظة: ' + data.governorate + '\n' +
                'العنوان: ' + data.address + '\n\n' +
                'تفاصيل الطلبية:\n' + (data.message || 'لا يوجد');

            const subject = 'طلب جملة جديد - ' + data.product_interest + ' - ' + data.governorate;
        ...
        .catch(err => {
            ...
            errBox.textContent = err.message
                || ('عذراً، تعذّر إرسال الطلب. يرجى المحاولة مرة أخرى' + (waDigits ? '، أو مراسلتنا عبر واتساب.' : '.'));
