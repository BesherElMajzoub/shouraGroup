...
@section('admin_content')
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-6 md:p-8 space-y-6 text-right">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" placeholder="مثال: custom-slug"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                            class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                        <option value="project" {{ old('type') === 'project' ? 'selected' : '' }}>معرض المشاريع</option>
                        <option value="news" {{ old('type') === 'news' ? 'selected' : '' }}>المركز الإعلامي (الأخبار)</option>
                    </select>
                </div>
