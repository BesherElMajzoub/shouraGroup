...
@section('page_title', 'تعديل قطاع: ' . $sector->name)

@section('admin_content')

    <div class="max-w-3xl mx-auto space-y-6">
        <a href="{{ route('admin.sectors.index') }}" class="inline-flex items-center gap-1.5 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">
        ...
        <form action="{{ route('admin.sectors.update', $sector->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="pt-4 border-t border-gray-100 flex items-center justify-between">
                ...
                @unless ($sector->is_coming_soon)
                    <a href="{{ route('sectors.show', $sector->slug) }}" target="_blank"
                       class="text-sm font-bold text-gray-500 hover:text-brand transition-colors">معاينة الصفحة ↗</a>
                @endunless
            </div>
        </form>
    </div>
