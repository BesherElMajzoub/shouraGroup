...
@section('title', 'تعديل حدث الخط الزمني')
@section('page_title', 'تعديل حدث في الخط الزمني')
...
@section('admin_content')
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.timeline.update', $timelineNode->id) }}" method="POST" class="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-6 md:p-8 space-y-6 text-right">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="event_date" name="event_date" value="{{ old('event_date', $timelineNode->event_date) }}" placeholder="1980" required
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $timelineNode->title) }}" required
                           class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" id="order" name="order" value="{{ old('order', $timelineNode->order) }}" required
                       class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
            </div>
