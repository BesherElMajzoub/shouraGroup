        ...
        <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-6 md:p-8 space-y-6 text-right">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" dir="ltr">
                ...
                <div class="text-left">
                    ...
                    <input type="text" id="dept_en" name="dept_en" value="{{ old('dept_en', $service->dept_en) }}" required
                           class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" id="whatsapp" name="whatsapp" value="{{ old('whatsapp', $service->whatsapp) }}" dir="ltr" placeholder="963932101176"
            ...
            <div>
                ...
                <select id="group" name="group" required
                        class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                    <option value="home" {{ old('group', $service->group) === 'home' ? 'selected' : '' }}>بطاقة مختصرة في قسم «الخدمات والحلول» بالصفحة الرئيسية</option>
                    <option value="pillar" {{ old('group', $service->group) === 'pillar' ? 'selected' : '' }}>خدمة تفصيلية في صفحة «خدماتنا وحلولنا»</option>
                </select>
            </div>
            ...
            <div dir="ltr" class="text-left">
                ...
                <textarea id="description_en" name="description_en" rows="4" required
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand resize-none">{{ old('description_en', $service->description_en) }}</textarea>
            </div>
            ...
            <div dir="ltr" class="text-left">
                ...
                <textarea id="features_text_en" name="features_text_en" rows="5"
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand resize-none">{{ old('features_text_en', $features_text_en) }}</textarea>
            </div>
