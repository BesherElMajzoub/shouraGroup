        ...
        <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-6 md:p-8 space-y-6 text-right">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="title_en" name="title_en" value="{{ old('title_en') }}" dir="ltr" required
            ...
            <div>
                ...
                <textarea id="excerpt_ar" name="excerpt_ar" rows="2" placeholder="اكتب ملخصاً قصيراً وجذاباً للخبر هنا..."
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all resize-none">{{ old('excerpt_ar') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="excerpt_en" name="excerpt_en" rows="2" dir="ltr" placeholder="Short, engaging summary in English..."
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all resize-none text-left">{{ old('excerpt_en') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="body_ar" name="body_ar" rows="10" required placeholder="اكتب تفاصيل الخبر هنا، يمكنك استخدام الفقرات والسطور الجديدة..."
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all resize-y">{{ old('body_ar') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="body_en" name="body_en" rows="10" dir="ltr" required placeholder="Write the full article in English here..."
                          class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all resize-y text-left">{{ old('body_en') }}</textarea>
            </div>
