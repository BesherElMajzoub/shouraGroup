...
@section('page_title', 'إضافة مشروع جديد للمعرض')
...
@section('admin_content')
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-6 md:p-8 space-y-6 text-right">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="client" name="client" value="{{ old('client') }}" placeholder="مثال: شركة س ص"
                           class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location" value="{{ old('location') }}" placeholder="مثال: الرياض"
                           class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="year" name="year" value="{{ old('year') }}" placeholder="مثال: 2026"
                           class="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                <!-- Image File -->
                <div>
                    <label for="image" class="block text-xs font-bold text-gray-700 mb-2">صورة المشروع الرئيسيّة</label>
